...
class InfectionModel:
    ...
    def hospital_beds(self, simulation_output_dir):
        df_r1 = self.df_progression_times
        df_r2 = self.df_infections
        ...
        survived = hospitalized[hospitalized.tdeath.isna()]
        FOUR_WEEKS = 28
        ...
        beds_availability = 100
        plt.plot(d_cases, np.ones(len(d_cases)) * beds_availability)
        legend = ['Hospital beds required', '# deceased cases', 'beds available']
        if sum(cumv > beds_availability) > 0:
            critical_t = df.t.values[cumv > beds_availability].min()
            plt.plot([critical_t] * 2, [0, max(d_cases.max(), cumv.max())])
            legend.append(f'Critical time {critical_t:.1f}')
        plt.legend(legend, loc='upper left')
        plt.title(f'[WIP] Hospital beds needed assuming 4 weeks for recovery \n {self._params[EXPERIMENT_ID]}')
        plt.savefig(os.path.join(simulation_output_dir, 'hospital_beds_analysis.png'))
    ...
    def icu_beds(self, simulation_output_dir):
        ...
        plt.close()
        cond = individuals_severity == ExpectedCaseSeverity.Critical
        critical = df_r1[df_r1.index.isin(cond.index)]
    ...
    def run_simulation(self):
        ...
        for i, seed in enumerate(seeds):
            ...
            if self._params[LOG_OUTPUTS]:
                self.log_outputs()
        intensity = self._params[TRANSMISSION_PROBABILITIES][CONSTANT]
        logger.info(f'\nconstant intensity not normalized: {intensity}'
                    f'\nconstant intensity normalized: {intensity/0.376}'
                    f'\nsimulation iterations: {len(eval(self._params[RANDOM_SEED]))}')
